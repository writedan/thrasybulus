            ...
            Ok(packet) => {
                ...
                if eth_packet.get_ethertype() == EtherType(0x0806) {
                    if let Some(arp_packet) = ArpPacket::new(eth_packet.payload()) {
                        ...
                        match opcode {
                            1 => {
                                println!("Who has {}? Tell {}", arp_packet.get_target_proto_addr(), arp_packet.get_sender_proto_addr());
                            },

                            2 => {
                                println!("{} is at {}", arp_packet.get_sender_proto_addr(), arp_packet.get_sender_hw_addr());
                            },
